...
def model_info(model): #显示模型参数量
    ...
    trainable_params = 0
    all_param = 0
    for param in model.parameters(): # 遍历model.parameters()返回的全局参数列表
        all_param += param.numel()
        if param.requires_grad:  # 可训练参数
            trainable_params += param.numel()
    print(f"[Model Info] All Params:{all_param} | Trainable:{100 * trainable_params / all_param}%\n")   
...
def get_metric(pred, seg, n_cls=4, iou_thresh = 0.5):
    ...
    #返回该批次模型判断的TP,FN,FP
    # n_cls = max(np.max(pred),np.max(seg)) #TODO
    TP,FN,FP = 0,0,0
...
#分类 两种思路，一个是借助，转变为分割 计算iou
#一个是直接计算对应度

def coord_metrics(pred, targ, n_cls=4, iou_thresh = 0.5, dist_thresh = 2):
    #pred:模型判断车道线的坐标，(bs,n_cls,n_anchors,2)
    #
    pass
